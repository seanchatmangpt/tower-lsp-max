...
"#;
    // Cursor is after "    f" on line 7 (0-based: line 7, character 5)
    ...
    // 2. Check ServerCapabilities false positive (cursor outside ServerCapabilities but closed with };)
...
"#;
    // Cursor is after "        t" on line 9 (0-based: line 9, character 9)
    ...
    // 3. Check rpc attribute completion when another parameter is first and #[rpc] is > 3 lines above
...
"#;
    // Cursor is after "textDoc" on line 6 (0-based: line 6, character 42)
...
fn test_methods_and_capabilities_table_consistency() {
    ...
    // 1. For every method with a capability_field, ensure it is in CAPABILITY_FIELDS
    for method in METHODS {
        if let Some(cap_field) = method.capability_field {
            // Find cap_field in CAPABILITY_FIELDS
    ...
    // 2. For every entry in CAPABILITY_FIELDS, ensure all listed methods exist in METHODS and map back
