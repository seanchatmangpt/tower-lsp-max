            ...
            for f in findings {
                ...
                let diagnostic = Diagnostic {
                    range: Range::new(
                        ...
                        Position::new((f.line - 1) as u32, (f.column - 1 + f.matched_text.len()) as u32),
                    ),
...
impl LanguageServer for PatternLsp {
    async fn initialize(&self, _: InitializeParams) -> tower_lsp_max::jsonrpc::Result<InitializeResult> {
        Ok(InitializeResult {
    ...
    async fn diagnostic(&self, params: DocumentDiagnosticParams) -> tower_lsp_max::jsonrpc::Result<DocumentDiagnosticReportResult> {
...
pub fn cmd_serve(stdio: bool) -> Result<ServerResult> {
    ...
    rt.block_on(async {
        if stdio {
            ...
            let (service, socket) = LspService::new(|client| PatternLsp::new(client));
