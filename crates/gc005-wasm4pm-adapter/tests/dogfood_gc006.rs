...
fn test_gc006_authority_surface_lock() {
    ...
    while !tower_lsp_max_root.join("Cargo.toml").exists() || !std::fs::read_to_string(tower_lsp_max_root.join("Cargo.toml")).unwrap().contains("[workspace]") {
    ...
    // gc-sealed-baseline
    {
        ...
        let check_clean = |repo_name: &str| {
            ...
            for line in status_out.lines() {
                ...
                if path == ".gc-sealed-baseline" || path.contains(".DS_Store") || path.starts_with(".claude/") || path.ends_with(".swp") || path.ends_with(".swo") {
                    continue;
                }
    ...
    // wasm4pm-lsp-no-local-conformance
    {
            ...
            let lsp_src = fs::read_to_string(tower_lsp_max_root.join("crates/wasm4pm-lsp/src/main.rs")).unwrap();
