...
## Overview
This report details the work completed by the Docs Writer to establish the documentation infrastructure for `lsp-max` in accordance with the post-human LSP doctrine. Seven files have been created or modified in total, structuring the project's documentation not as onboarding tutorials, but as formal state-machine projections (Docs as Law Projections).
...
1.  **`docs/law/post-human-lsp-frame.md` (Created)**
    ...
    *   *Purpose:* Formally declares LSP as a post-human project-state protocol. Explicitly states that `lsp-max` operates as a state transition gateway rather than a developer IDE assistant. Establishes the "Docs as Law Projections" concept and documents the server's four architectural layers.
...
3.  **`docs/law/law-state-runtime-primitives.md` (Created)**
    ...
    *   *Purpose:* Documents the typestate machine wrapper `Machine<L, P, D>` from `lsp-max-runtime` and maps the semantic phases (`UninitializedState` to `ExitedState`). Explains process exit codes and details cryptographic receipt chain ledger replay.
...
5.  **`docs/adr/ADR-0001-lsp-max-purpose.md` (Updated/Rewritten)**
    *   *Title:* ADR-0001: Bootstrap Specification Generator First (Refined)
    *   *Purpose:* Refined to focus on the decision to repurpose the workspace framework as `lsp-max` (a post-human project-state server) rather than a simple IDE helper.
6.  **`docs/adr/ADR-0002-generated-protocol-vocabulary.md` (Created)**
    ...
    *   *Purpose:* Outlines the decision to generate standard and extended protocol vocabulary using `lsp-max-specgen` directly from `metaModel.json` to prevent manual drift.
...
- **Typestate Machine:** In `law-state-runtime-primitives.md`, we outline how the compile-time checks in `lsp-max-runtime` prevent invalid state transitions by design.
